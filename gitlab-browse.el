...
(defun gitlab-kill ()
  "Add URL for current selection to killring, if applicable.
   If a Dired buffer is open, create a url to view to the directory listing in gitlab.
   If the current buffer is neither a file or Dired buffer, gets a url for the current
   project in gitlab"
  (interactive)
